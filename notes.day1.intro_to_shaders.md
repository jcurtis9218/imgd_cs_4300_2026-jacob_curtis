...
[The Schwartz](https://charlieroberts.codeberg.page/TheSchwartz/) is an environment for live coding WGSL shaders.
We'll use it to do some initial WGSL experiments creating waveforms.
...
The Schwartz comes pre-programmed with special **uniforms** (a uniform is a value transferred from the CPU to the GPU). One such value is `frame`. We can use this to make a simple sine wave:
...
However, you’ll notice that this spends a half of each cycle as black, because the `sin()` function returns values between -1 – 1. 
We can provide a bias and a scalar to get this to a range of 0–1:
...
As we mentioned earlier, the `pos` variable reports the current fragment position in pixels. The Schwartz provides
the uniform `res` to tell you the resolution of your drawing surface. We can use these together to get a
normalized (0--1) coordinate for our fragment, and can use this to make a classic video oscillator on our screen.
...
What happens when you multiply `p.x` by `p.y` instead of dividing it? 
What happens when you multiply the entire output of the `sin()` function by `p.y`?
